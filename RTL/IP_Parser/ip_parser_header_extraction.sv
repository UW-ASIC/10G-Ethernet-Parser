...
logic [1:0] beat_cnt;
// this is the cycle dst_ip gets WRITTEN below, but it's a flop - the new
// value isn't actually sitting in dst_ip until the next edge. registering
// this into capture_done (below) means capture_done pulses when dst_ip is
// really there, instead of handing that timing gotcha to whoever reads us
logic dst_ip_capturing;
assign dst_ip_capturing = beat_valid && (beat_cnt == 2'd2);
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // registered a cycle behind dst_ip_capturing on purpose - see comment above
        capture_done <= dst_ip_capturing;
        if (beat_valid) begin
